...
namespace Algorithm
{
    public class Finder
    {
        ...
        public Result Find(FindTypes findType)
        {
            ...
            {
                TimeSpan span = orderedPersons.Last().BirthDate.Subtract(orderedPersons.First().BirthDate);
                result = new Result { Difference = span, Person1 = orderedPersons.First(), Person2 = orderedPersons.Last() };
            }
            else
            {
                var processedPersons = new List<Result>();

                for (int i = 0; i < orderedPersons.Count - 1; i++)
                {
                    TimeSpan span = orderedPersons[i + 1].BirthDate.Subtract(orderedPersons[i].BirthDate);
                    processedPersons.Add(new Result { Difference = span, Person1 = orderedPersons[i], Person2 = orderedPersons[i + 1] });

                }
                result = processedPersons.OrderBy(p1 => p1.Difference).First();
            }
            return result;
